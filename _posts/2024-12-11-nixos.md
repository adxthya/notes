...
- Which is why we have nix-direnv.
- Nix-Direnv will activate the dev shell everything you switch to a folder with a `flake.nix` or `shell.nix`.
### NVF
...
### NH
- When you update your system the default way using `sudo nixos-rebuild switch`, you would be seeing much info on what changed during the update.
- [NH](https://github.com/nix-community/nh) allows you to get a better understanding on what all is being done during the switch and what packages were removed/installed after the switch.
- You can also set NH to automate the task of removing the generations.
